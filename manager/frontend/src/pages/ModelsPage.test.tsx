...
describe('ModelsPage', () => {
  ...
  describe('browse model details', () => {
    ...
    it('fetches model info for HuggingFace models', async () => {
      mockClient.getModelInfo.mockResolvedValueOnce({
      ...
      await waitFor(() => {
        expect(mockClient.getModelInfo).toHaveBeenCalledWith('hf/model1');
      });
    });
    ...
    it('fetches model info for ModelScope models', async () => {
      ...
      await waitFor(() => {
        expect(mockClient.getModelInfo).toHaveBeenCalledWith('modelscope/ms-model');
      });
    });

    it('handles model info fetch error gracefully', async () => {
      mockClient.getModelInfo.mockRejectedValueOnce(new Error('Fetch failed'));
      mockUseBrowse.mockReturnValue({
        ...defaultBrowseHook,
        source: 'huggingface',
        models: [{
