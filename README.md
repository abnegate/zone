...
`ZONE_CHAT_ROUNDS` (64) and `ZONE_CHAT_CALLS` (256) bound a turn, with a final tool-free response when a budget or unchanged-evidence cycle ends execution. `ZONE_CHAT_TIMEOUT_SECONDS` (1800) bounds elapsed generation time and preserves interrupted evidence on expiry. `ZONE_CHAT_OUTPUT_TOKENS` (4096) is the maximum response reservation; known small context windows reserve at most one quarter, so a 4096-token model receives `max_tokens=1024`. The same reservation applies to preview, generation, and summaries. `ZONE_CHAT_RECHECK_PINGS` (200) is how many keepalive pings an open chat socket serves between authorization rechecks, so a member whose access is revoked mid-conversation is disconnected within roughly that many ping intervals. These settings must be positive integers; invalid settings fail server startup.
