...
5. **Ask-user tool** (H1) for chat, then the task waiting state. Landed in PR 5. The tool ends the turn when it is called, and the answer the reader picks arrives as the next user message rather than as a frame of its own. In chat the card is streamed as it is asked and stored on the tool call, so it survives a reload. A background run parks instead: the run goes to `waiting` with the question in `task_runs.pending_question`, and any workspace member with write access can answer it. When every question is optional the run proceeds on the recommended option after about 30 seconds; a required question has no window, because defaulting past it would decide the thing the model said it could not. The wait sits inside the run's existing one-hour timeout, and that timeout is terminal — the run fails and is not retried.
