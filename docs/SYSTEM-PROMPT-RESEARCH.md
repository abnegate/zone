...
8. **Automations** (I1). The recurring half landed in PR 8: a reminder may carry an `rrule`, and one that does moves to its next firing rather than ending. Two limits a reader meets first — hourly is the ceiling, and a rule this build does not implement is refused rather than silently dropped, so a schedule never fires on a day nobody chose. A firing can now also run a turn: a reminder carrying a `prompt` delivers the model's answer rather than fixed words, through the same generation path a typed message takes, and the firing is written down before the schedule moves past it so a restart mid-turn costs a repeated question rather than a missing answer. What is left is `condition_watch` and `flexible_schedule`, which need a firing to compare state against the last one and to carry the edge-blind statement — that a watch cannot see a change appearing and disappearing inside one interval. Both stay refused until they work, because a `condition_watch` that only redelivers is not a watch.

    The tempting shortcut for the comparison is to take nothing: a watch fires in the same chat every time, so the last firing's answer is already in the history the model is given, and it could be asked to compare against it. That is wrong, and quietly so. The history is compacted, and an hourly watch outlives its own baseline within a day — after which the model is asked to compare against something that is no longer in front of it, and has no way to say so. It would report a change on the first firing after a compaction and call it news. So the last observation has to be kept on the row and handed to the firing explicitly, capped, which is a column and a write in the worker rather than something the chat gives away for free. A watch must also be refused unless it carries both an `rrule` and a `prompt`: one that fires once has nothing to compare against, and one with fixed content has nothing to compare.
