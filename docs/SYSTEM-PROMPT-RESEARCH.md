...
| I1 | Recurring and condition-watch automations. **Half landed in PR 8**, the recurring half: `create_reminder` takes an `rrule` in an owned RFC 5545 subset (`FREQ`, `INTERVAL`, `BYDAY`, `BYHOUR`, `BYMINUTE`, `BYMONTHDAY`, `UNTIL`, `COUNT`), a clause outside it is refused rather than dropped, hourly is the ceiling counted after the `BY` clauses split the period, and `deliver_next` moves a recurring row to its next firing instead of settling it. The anchor is the first firing so a late tick cannot make the schedule drift; the search starts from now so a firing slept through is not replayed; every computed occurrence carries a deterministic offset from the automation's own id, and the first firing carries none because it is the time the person named. A recurring row stops after seven days unless asked for again and ends as `expired`, which a reader can tell from `delivered` and from `cancelled`. **A firing can now run a turn.** A reminder carrying a `prompt` stores no message of its own: the dispatch hands the turn back once its transaction commits and the worker runs it through `ws::chat::run_turn`, which persists the prompt as the user message and the model's answer beside it. No socket is involved because none was ever needed — `ChatStream` keys a broadcast channel by chat id and drops a frame nobody is subscribed to — so an automation's turn takes the same lease, the same per-chat serialisation and the same recovery as a typed one. The prompt is marked with its reminder, so a reader can tell it from something the person typed. **`condition_watch` and `flexible_schedule` still did not ship**: a watch needs to compare state between firings and carry the edge-blind statement saying it cannot see a change that appears and disappears inside one interval, and neither exists yet, so both stay refused at the create and at the database | Partly done | `db/reminders.rs`; `services/schedule.rs`; `agent/actions.rs`; `035_reminder_automations.sql` | SOL 707-876, CC 943-1014 |
