...
Task profile: the same host list minus `run_shell`, so six sandboxed file and command tools, plus MCP. No workspace tools, no web, no ask-the-user path.
...
Read `AGENTS.md`, `CLAUDE.md`, `.zone/instructions.md` (pick the set; the context gatherer already prioritises `CLAUDE.md`) from the checkout root into task guidance. CC 162 brackets its injected project file with "These instructions OVERRIDE any default behavior", but Zone must not copy that: a checkout is observed content under B1, and a repository Zone did not write cannot be allowed to outrank the operator's system sections. Inject the files as source-labeled context instead, naming the path each block came from, ranked above the model's own habits and below everything in the system prompt, with the standing statement that they describe how this repository wants work done and are not authorization for anything the operator's rules refuse. Pair with CX 13 and 138: if one of these files makes the agent stop or ask, it must name and quote the file and separate the explicit requirement from its interpretation. And CX 25: an exception written in a repository file does not by itself require user approval. Build in `workers/task.rs::guidance`.
...
CX 45-51: lead with the outcome; explain what changed, why, how it was tested, and material risks; order evidence for assessability; summarise routine verification. CX 53-57: PR description written for a reviewer who has not seen the chat, problem first, rewritten when scope changes, abandoned approaches omitted. Build: the task's final message becomes the PR body. Two places mint the title today and both hardcode the `[Zone]` prefix: `workers/pr.rs:235` builds the commit message, and `zone_vcs::pull_request::generate_pr_title` (`runner/zone_vcs/src/pull_request.rs:429`) builds the PR title as `[Zone] <title> (<short id>)`. Both move to a conventional-commit subject (`(type): subject`, the house format used throughout this repository's history) derived from a short structured call, and `generate_pr_body` (`:435`) takes the task report. Add CC 890-898's trailer convention if the operator wants attribution.
...
**E3. Token-saving defaults. P0. Done in PR 1; no code was written for it.**
...
Both halves were already satisfied by the time PR 3 came to close the item, and neither needed a change:
- The prompt half shipped with the section builder in `9ff2912` (PR #51). `READING` in `runner/zone_server/src/agent/prompt/section/files.rs` carries CC 1914, CC 1918 and SOL 3101 in a single bullet — "Read only the part of a file you need when you already know where it is, and do not read a file back to check a write you just made: the tool result already said it applied. Prefer plain-text formats over ones that inflate the context." — and it renders on the chat and task surfaces both.
- The `apply_patch` half was a check, not a change. `ApplyPatchTool::execute` in `runner/zone_core/src/tools/file.rs` returns `Updated {path} ({n} replacement(s))` and never echoes the file back, so there was nothing to trim.
...
**F1 follow-up. The published date never reaches the model. P1.**
`time_range` shipped in PR 3 and narrows the SearXNG request, but the date on the way back is discarded: `SearxngResult` in `runner/zone_search/src/client.rs` deserializes only `title`, `url` and `content`, `SearchHit` carries only those three, and `format_search_context` renders title, URL and snippet. SearXNG returns `publishedDate` for most engines, so this is thrown away rather than unavailable. Two consequences, both of which undercut what PR 3 just built. The model cannot tell whether a recency-narrowed search actually worked, because a `time_range: day` result set looks exactly like an unnarrowed one, so it cannot judge whether to narrow further or give up. And it cannot follow two rules the shipped prompt already gives it: "Time-sensitive answers need a source explicitly dated recently" has no date to read, and "Drop a source you are unsure of rather than hedge it" becomes a choice between dropping everything and dropping nothing. Carry `publishedDate` through `SearxngResult` into `SearchHit` and render it in `format_search_context`.

**F2. Typed citation IDs. P1.**
GK 758-765: cite only through IDs the tools returned (`[web:id]`, `[post:id]`, `[collection:id]`), placed after the final punctuation of the sentence or cell; structured-data tools are exempt. SOL 422-450: typed reference IDs shared by citations and rich UI; never write raw markdown links; one citation block per paragraph. CL 7728: claims in your own words, citation tags are attribution not permission to quote. Build: tag every `web_search`, `search_knowledge`, `read_document` and `search_chat_history` hit with a stable ID in the tool output, instruct the model to cite `[web:3]`, and resolve IDs server-side into `Citation` records in `citations.rs`, rejecting unknown IDs. Server-side resolution only closes the path it owns: the console renders assistant text through react-markdown, so a raw markdown link or a bare autolink in the reply reaches the user without ever passing the citation check. Carry SOL 449 into both halves. Prompt: never write a markdown link or a bare URL, cite by ID. Renderer: `MessageContent` rejects or de-links any anchor whose target does not resolve to a `Citation` on that message. Without the renderer half this item makes invented URLs look validated rather than making them impossible. `query_prometheus`, `get_build_status` and the other structured tools stay citation-exempt (GK 762).
...
SOL 707-734 is a complete contract: the schedule is an iCal `VEVENT` with `RRULE`, and the subset SOL itself names carries no `SUMMARY`, no `DTEND`, and no `UNTIL` or `COUNT` to stop with — the subset this build shipped is wider than that and is the one in §3's I1 row; relative one-shots use an offset; three timing modes (`exact_schedule`, `flexible_schedule` with daypart defaults, `condition_watch` which must be recurring and polls as fast as the condition can change); hourly is the ceiling; the stored prompt is an imperative to the future self and carries "if nothing changed, do not notify me". Worked examples at SOL 736-813, the suggest-don't-create policy at SOL 815-827, and `list` versus `peek` at SOL 863-876. CC 943-1014 adds fleet-friendly scheduling: avoid `:00` and `:30`, deterministic jitter up to 10 percent of the period, recurring jobs expire after seven days unless renewed. Build: extend `reminders` with `rrule`, `prompt` and `timing_mode`, execute a chat turn or a task on fire in the reminders worker, keep the 10 s poll as the dispatch tick. That tick decides when a due automation runs, not how often the watched thing is sampled: a `condition_watch` fires on its own schedule, and between two firings it sees only the state at each firing. So the contract is edge-blind by construction. State it rather than implying otherwise, in three places. The tool description says a watch compares the state at each run against the last run and reports a difference, so a condition that appears and disappears inside one interval is not detected. The stored prompt keeps SOL 739's "if nothing changed, do not notify me" shape, which is what makes a missed transient harmless rather than silently wrong. And a watch whose subject emits real events (a GitHub check, a task run, a webhook) is a trigger, not a poll, and belongs on the event instead, which is the same primitive as I2. Prompt: `create_reminder` gains the SOL rules; the chat prompt gets "suggest an automation after fast-changing-info requests, never create one unasked".
...
**L2. Edit target verification. P1.**
...
**L3. Content guard travels with the tool. P1.**
...
| F1 | When to search, recency rules | P0 text, P1 parameter | web section; `time_range` in `zone_search` | SOL 316-356, 442-444, 579-583, CL 206-208, 1709-1725 |
...
| L2, L3 | Image edit target check; content guard in tool descriptions | P1 | `images.rs`, `audio.rs` | SOL 1252-1256, GK 362, 391 |
...
| E2 | Background commands, `tail_job`, output cap. Landed in PR 6, with the output cap already in from PR 3: `background: true` on `run_shell` and `run_command` detaches the command and returns a job id and a log path under the session's working directory, and `tail_job(id, since)` reads the log from a byte offset, under the same `max_output_chars`, ending with the job's state. A job is killed after 900 s, the shell cap, or once its log passes 64 MiB, and it ends with the turn or the run that started it. The completion event in the build line did not ship as an event: the loop is re-invoked on a job's exit only through `wait_for` (I2); otherwise a chat learns how its jobs ended at the turn's teardown, and a task run kills what it left running | Done | `zone_core/src/tools/job.rs`, `command.rs`, `tail.rs`; `ws/chat.rs` job frames | CC 881-941, GK 591-633 |
| E3 | Token-saving defaults. Done, no code: the prompt half shipped in PR 1 (`9ff2912`), and `apply_patch` was checked and does not echo the file | Done | file section; `apply_patch` result | GK 551, CC 1914, 1918 |
...
| H1 | `ask_user` tool and question card. Landed in PR 5, both halves: the chat card and the task waiting state. The schema takes up to four questions and the prompt asks for one, three at most. A background run parks in `waiting` until an owner, admin or member answers, never a viewer; when every question is optional it proceeds on the recommended option after 30 seconds, and a required one waits inside the run's one-hour timeout, which fails terminally and is not retried | Done | `agent/question.rs`; `AgentEvent::QuestionRequired`; `ws/chat.rs`; `workers/task.rs`; `task_runs.status = 'waiting'` and `pending_question` | CC 744-879, CL 3030-3055, CX 65, CL 2750 |
| E4 | Deferred tool loading | P2 | `ChatTools`; new `search_tools` and `load_tools` | CC 2448-2482, GK 433-486, SOL 1153 |
| G1 | Model-writable memory with profile and preferences. Landed in PR 7: five `memory_*` tools over `knowledge_entries` with three per-user categories — `profile` and `preference` hold one entry each with the title forced, a `fact` is named and described — and a `version` column a write or a delete echoes, a zero-row result being the conflict that hands back the current content to merge. Profile and preferences render for every reader on both prompt surfaces, a chat with its agent off included, because they describe the person rather than point at a tool; the fact index and the shortening notice name `memory_read`, so they render only where it is registered, which is a chat with its agent on. The tools are registered in a chat only, so a run reads what is remembered and cannot write it. Isolation is one predicate, `category NOT LIKE 'memory-%'`, negated into every workspace-wide read including `update_document` and the embedding backfill, plus a `404` on the two knowledge routes that take an id and a refusal on the category at the knowledge post; nothing is embedded, so `search_knowledge` cannot return one. One rulebook refuses credentials, contact details, account numbers and suppress-honesty instructions for the write tools and for the promotion worker, which counts what it withheld; the sensitive categories are prompt-only, because a server cannot detect them. An entry holds 2 000 characters, refused rather than truncated, and the rendered block 6 000 bytes, saying what it cut. The create is a `NOT EXISTS` check over a partial unique index on the workspace, owner, kind and name of every active remembered entry, so two exactly-overlapping first writes cannot both insert: the loser's unique violation is read as the same conflict a stale version gets, carrying what the winner stored | Done | `db/memory.rs`; `agent/memory/{mod,rules,render}.rs`; `agent/prompt/section/memory.rs`; `db/knowledge.rs` `not_memory!` and `PRIVATE_CATEGORIES`; `routes/context.rs` `private`; `workers/promotion.rs` `screen`; `ws/chat.rs` `memory_used`; `033_knowledge_memory.sql`; `034_knowledge_memory_entry_index.sql` | CC 44-65, CL 297-1018, CL 2159-2384, SOL 58, 1097-1123 |
| I1 | Recurring and condition-watch automations. **Half landed in PR 8**, the recurring half: `create_reminder` takes an `rrule` in an owned RFC 5545 subset (`FREQ`, `INTERVAL`, `BYDAY`, `BYHOUR`, `BYMINUTE`, `BYMONTHDAY`, `UNTIL`, `COUNT`), a clause outside it is refused rather than dropped, hourly is the ceiling measured at the shortest gap the rule actually produces rather than at the average one, and `deliver_next` moves a recurring row to its next firing instead of settling it. The anchor is the first firing so a late tick cannot make the schedule drift; the search starts from now so a firing slept through is not replayed; every computed occurrence carries a deterministic offset from the automation's own id, and the first firing carries none because it is the time the person named. A recurring row stops after seven days unless asked for again and ends as `expired`, which a reader can tell from `delivered` and from `cancelled`. **A firing can now run a turn.** A reminder carrying a `prompt` stores no message of its own: the claim writes the turn down as still owed, in the same transaction that moves the schedule on, and the worker runs it from there through `ws::chat::run_turn`, which persists the prompt as the user message and the model's answer beside it. Written down rather than handed back in memory, because the schedule has already moved past the occurrence by the time the claim commits, so a process that died in between would leave a firing nobody was ever given; a turn whose worker did not live to mark it done is offered again once its claim goes stale, and given up on after three attempts rather than retried into a crash loop. No socket is involved because none was ever needed — `ChatStream` keys a broadcast channel by chat id and drops a frame nobody is subscribed to — so an automation's turn takes the same lease, the same per-chat serialisation and the same recovery as a typed one. The prompt is marked with its reminder, so a reader can tell it from something the person typed. **`condition_watch` now ships too.** A watch is the same firing handed what the last one answered and asked what differs. The reading is a column on the row and an explicit write, not something the chat gives away: both firings land in the same chat, so the previous answer is already in the history the model is given, and taking it from there fails silently once that history is compacted — an hourly watch outlives its own baseline within a day and would then report the summary losing detail as news. The reading is capped, which is safe where compaction was not, because the same leading characters are kept every firing and two readings stay comparable. A watch is refused without both an `rrule` and a `prompt`: one firing has nothing to compare against and fixed content has nothing to compare. A firing that stored no answer leaves the baseline alone rather than overwriting it with nothing, which would report the world changed when only that firing failed. Two limits are stated rather than implied — the watch sees only the state at each firing, so a change that appears and disappears between two of them is never noticed, and an unchanged firing still answers in the chat, in one short line, because running the turn is how a watch reports at all. **`flexible_schedule` still did not ship**: placing a firing inside a window needs a reading of what else is on the person's day that nothing here takes, so it stays refused at the create and at the database | Partly done | `db/reminders.rs`; `services/schedule.rs`; `agent/actions.rs`; `workers/reminders.rs`; `ws/chat.rs`; `035_reminder_automations.sql`; `037_reminder_turns.sql`; `038_reminder_condition_watch.sql` | SOL 707-876, CC 943-1014 |
| I2 | `wait_for` primitive. Landed in PR 6: `wait_for(kind, id, ref, timeout_secs)` waits server-side on a background job's exit, a task run's ending or a commit's checks. The call ends the turn with a receipt; the outcome is injected as the loop resumes, with the iteration refunded. A task run parks in `waiting` with the wait in `task_runs.pending_wait` and hands its admission slot back; a chat awaits inside its own turn and continues in the same message, and the turn survives a disconnect. Waits run 15 to 1800 s, 300 by default, ten per chat turn or task attempt. A timeout, a commit nothing reports on and checks that could not be read are all worded so they cannot read as a pass. Checks are polled every 30 s against the resolved commit, since no check webhook exists; `kind=task_run` is chat-only | Done | `agent/wait.rs`; `AgentEvent::WaitRequired`; `prompt/section/waiting.rs`; `workers/task.rs` park; `ws/chat.rs` second run; `task_runs.pending_wait` | CC 888, 1688-1820, 2107-2162 |
...
5. **Ask-user tool** (H1) for chat, then the task waiting state. Landed in PR 5. The tool ends the turn when it is called, and the answer the reader picks arrives as the next user message rather than as a frame of its own. The schema takes one to four questions, which is the ceiling the tool enforces; the prompt aims the model lower, at one question with three as its own ceiling, so the limit the reader ever sees is the guidance and not the schema. In chat the card is streamed as it is asked and stored on the tool call, so it survives a reload. A background run parks instead: the run goes to `waiting` with the question in `task_runs.pending_question`, and an owner, admin or member can answer it — a viewer may read the run but never answer for it. When every question is optional the run proceeds on the recommended option after about 30 seconds; a required question has no window, because defaulting past it would decide the thing the model said it could not. The wait sits inside the run's existing one-hour timeout, and that timeout is terminal — the run fails and is not retried.
6. **Background jobs and wait primitive** (E2 background mode, I2). Landed in PR 6. `background: true` on either shell tool detaches the command and returns a job id and a log path under the session's own working directory, and `tail_job` reads the log from a byte offset and says where the job has got to. `wait_for` covers a job's exit, a task run's ending and a commit's checks, and it is the only thing that resumes the loop on a job's exit — the exit itself wakes nothing. The call ends the turn with a receipt, the outcome is injected when the loop resumes with the iteration refunded, a task run parks in `waiting` and gives its slot back, and a chat awaits inside its own turn and continues in the same message. Two limits a reader meets first: a background job ends with the turn or the run that started it, or after 900 seconds, whichever comes first, so no job survives into a later turn; and `kind=task_run` is chat-only — a task run may wait on a job or on checks, and is told to finish and let whoever started it coordinate.
7. **Memory** (G1). Landed in PR 7. Five `memory_*` tools over `knowledge_entries` with three per-user categories: `profile` and `preference` hold one entry each with the title forced, and a `fact` is named and described so a later turn can tell whether to read it. A `version` column carries the compare-and-swap token — an integer rather than CL's twelve-character opaque one, because section 4 refuses a custom grammar on the grounds that local models mangle them, and an integer is a thing a model can echo back — so a write and a delete quote the version a read returned, and a zero-row result is the conflict that hands back the current content to merge. An append quotes none: it reads, composes and writes against the version it read, so an append that raced another is told it conflicted and composes again from what it then reads — nothing is lost, and the order the two additions land in is unspecified. The tools are chat-only and the rendered profile-and-preferences block reaches the task surface as well, so a run acts on what is remembered, never writes it, and is given no index because it has no `memory_read` to act on one with. Isolation is one predicate, `category NOT LIKE 'memory-%'`, negated into every workspace-wide read — the two listings, `documents!()`, keyword search, `update_document`, the embedding backfill and the chat's knowledge preview — with a `404` on the two knowledge routes that take an id, to the owner as well as to a stranger, and a refusal on the category at the knowledge post; nothing is ever embedded, so `search_knowledge`, which joins the embeddings table, has nothing to return. `agent::memory::rules` is the half of the exclusion list a server can check — credentials, contact details and account numbers, an instruction to hold something back — screening a fact's description as well as its content, and the promotion worker calls the same function over a candidate's question as well as its answer and counts what it withheld. The sensitive categories live in the prompt section alone, because a keyword list for them would refuse "remember I prefer tabs" for carrying a banned word and would miss every disclosure phrased otherwise. An entry holds 2 000 characters and is refused rather than truncated; the rendered block holds 6 000 bytes, cutting the fact index first with a line saying how many entries it omitted and only then shortening preferences and the profile at a character boundary, each saying so; the three writes mint `ActionReceipt`s the way a task or document write does; and the badge reads "Memory read" rather than "memory used", because a successful `memory_read` in the turn is what the server can derive, not whether the answer changed. Deliberately not taken: CC's `feedback` and `project` types, which `repository-convention` and `strategy-lesson` already derive server-side with provenance, so model-writable twins would give a workspace two answers to one question with no precedence rule; CC's paths and `[[links]]`, per section 4's "take the schema, not the paths"; CL's twelve-character token, for the reason above; and a console page with an `/api/memory` route — memory is reached through the tools that own it, which is why a memory receipt carries no link. The create is a `WHERE NOT EXISTS` check over a partial unique index on the workspace, owner, kind and name of every active remembered entry, so two exactly-overlapping first writes cannot both insert: the loser's unique violation is read as the same conflict a stale version gets, carrying what the winner stored. One limit shipped with it: the promotion path enforces only the mechanical half, with the occurrence and agreement thresholds upstream doing what the prompt does in a chat.
8. **Automations** (I1). The recurring half landed in PR 8: a reminder may carry an `rrule`, and one that does moves to its next firing rather than ending. Two limits a reader meets first — hourly is the ceiling, and a rule this build does not implement is refused rather than silently dropped, so a schedule never fires on a day nobody chose. A firing can now also run a turn: a reminder carrying a `prompt` delivers the model's answer rather than fixed words, through the same generation path a typed message takes, and the firing is written down before the schedule moves past it so a restart mid-turn costs a repeated question rather than a missing answer. `condition_watch` landed next, on the same primitive: a firing handed the last one's answer and asked what differs. What is left is `flexible_schedule`, which needs a window to place a firing inside and a reading of the person's day to place it against, and it stays refused until it works — a mode that quietly does less than its name is worse than one that is not offered.

    The tempting shortcut for the comparison was to take nothing: a watch fires in the same chat every time, so the last firing's answer is already in the history the model is given, and it could be asked to compare against it. That is wrong, and quietly so. The history is compacted, and an hourly watch outlives its own baseline within a day — after which the model is asked to compare against something that is no longer in front of it, and has no way to say so. It would report a change on the first firing after a compaction and call it news. So the last observation is kept on the row and handed to the firing explicitly, capped: a column and a write in the worker rather than something the chat gives away for free. The cap is not the same bargain as compaction. It keeps the same leading characters every time, so two readings stay comparable and what falls past it is missed consistently rather than intermittently, which is a limit of the same kind as the edge-blindness already stated. A watch is refused unless it carries both an `rrule` and a `prompt`: one that fires once has nothing to compare against, and one with fixed content has nothing to compare.

    Two things the built version had to decide that the design note did not. The answer is read back by the id the turn was given rather than by taking the chat's most recent message, so something the person typed while the turn was running cannot be kept as the watch's reading — which meant `run_turn` returning the id it allocates. And a firing that stored no answer at all leaves the baseline as it was: overwriting it with nothing would make the next firing compare against an empty reading and report everything as changed, when all that happened is that one firing did not run. The reading is also read live at run time rather than snapshotted onto the queued turn, so two firings that piled up behind a restart are compared against each other rather than both against the one reading they shared.

    One limit the contract asks for and this build cannot give. SOL 739's shape is "if nothing changed, do not notify me", and here running the turn *is* the delivery — there is no channel a firing can decline to use, because the answer lands in the chat as the turn's own message. So it degrades to one short line rather than to silence, and the firing is told to keep it to one. Stated in the tool description rather than left for somebody to discover, on the same principle as the edge-blindness: a watch that quietly reports every firing teaches the person to ignore it, and one that says so up front does not.
9. **Deferred tool loading** (E4). Landed in PR 9. A chat assembles fifty-odd tools, and every schema went out on every round of every turn. Measured with `zone_core::context::estimate`, the function that fills `ContextBreakdown.tools` in production, that block is 9,086 tokens on the merged tree — more than a quarter of a 32k window spent before the conversation starts, most of it on tools the turn was never going to touch. A core set is now always sent and the rest are listed in the system prompt as one name and one line of purpose, taken from the description the tool already declares rather than from a second list written beside it, because two lists drift and only one of them is what the model is handed when the schema arrives; `search_tools` finds a tool by what it does and `load_tools` brings its schema in. `definitions()` is recomputed each round rather than cached, so a load in one round is callable in the next round of the same turn, not the next turn — which is what makes deferring a tool the prompt names safe: no rule becomes unsatisfiable, what deferral costs is one round trip. On the real tool set for a real workspace the block is 4,845 tokens after the split, 4,241 saved — 47% of the tool block and 12.9% of the whole window — with 27 schemas sent and 20 listed by name. `deferred_tool_tests` asserts a floor rather than that figure, so editing a tool description does not fail the build; it prints what the saving is made of, because a share alone does not say whether deferral bought one heavy schema or twenty light ones.

    What stays in front of the model is chosen by how often a turn needs a tool against what its schema costs, not by what the prompt happens to name: host tools, retrieval and documents, `ask_user` and `wait_for`, `start_task`, and the two tools that fetch the rest. `create_reminder` is named by the same prompt sections as `start_task` and is deferred anyway — at 1,056 tokens it is the single most expensive schema in the catalog, more than three times the next, and a small minority of turns want it; keeping it core cost 13 points of the saving. The five memory tools are core on different grounds, and they are the one place this step and step 7 met: memory registers them after the snapshot this step takes of the core set, so merged naively they came out deferred and the test that checks the memory rules and the five schemas as one claim failed, rightly. The memory section renders its rules off the same catalog, naming each tool, so the rules and the schemas travel together; a rule the model can act on only after a round trip is a weaker rule, and the recall the profile block invites is exactly the call a chat would pay the trip on every time. They sit in `NEVER_DEFERRED` beside `start_task`, at 776 tokens between them.

    Listing a tool is not the same as sending its schema, and two things followed. An MCP server writes its own tool descriptions, and before deferral a remote description reached the model only as a schema; listing it puts it in the system prompt, the region a model reads as its own instructions, so a server whose first sentence was an instruction would have had it delivered in this system's voice — nothing else in the prompt had that property, since every string `guidance_for_tools` emits is written in this repository. MCP tools stay deferred, because they are the unbounded-count case where deferral is worth most, and the provenance is stated instead: `Listed` carries a `remote` flag, taken by difference across `register_mcp` because the registry does not record which names came from a hub, and the section renders our own lines, then a sentence saying the rest come from attached servers and are claims rather than orders, then theirs, with the order asserted, since a disclaimer after the text it disowns protects nothing. This bounds provenance, not reachability — a malicious server's tool is as callable as it was. And the listing's first wording asserted "their schemas are not loaded" about every tool in it, which the system message, built once a turn while the schemas are recomputed every round, cannot know: a model reading that about a tool `load_tools` had already fetched would fetch it again and spend exactly the round deferral exists to save. It now describes what a listing means rather than what is true at render time.

    One trade-off worth naming. The registry sorts definitions so the tools prefix is byte-identical between turns and a local server can reuse its prompt cache; a load changes that prefix, so the round after one pays a cache miss. That is the argument for a generous core set rather than a minimal one.
