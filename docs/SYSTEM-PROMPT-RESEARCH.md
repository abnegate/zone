...
8. **Automations** (I1). The recurring half landed in PR 8: a reminder may carry an `rrule`, and one that does moves to its next firing rather than ending. Two limits a reader meets first — hourly is the ceiling, and a rule this build does not implement is refused rather than silently dropped, so a schedule never fires on a day nobody chose. What is left is the half that needs a firing to run a turn rather than deliver fixed content: the stored `prompt`, `condition_watch` and `flexible_schedule`, and the edge-blind statement a watch has to carry in three places. Those stay out of the tool schema until they work, because a `condition_watch` that redelivers the same content is not a watch.
