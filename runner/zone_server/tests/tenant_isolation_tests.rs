...
async fn test_user_cannot_see_other_workspace_chats() {
    ...
    let chat1 = chats::create_chat(&pool, Some(ws1.id), "Chat 1", "gpt-4", false, true)
        .await
        .expect("Failed to create chat 1");

    let chat2 = chats::create_chat(&pool, Some(ws2.id), "Chat 2", "gpt-4", false, true)
        .await
...
async fn test_workspace_deletion_cascades() {
    ...
    let chat = chats::create_chat(&pool, Some(ws.id), "Chat", "gpt-4", false, true)
        .await
