...
/// Fold the tool trace and citations into the image metadata, since one turn
/// can produce all three and they share the message's single metadata column.
fn merge_metadata(
    images: Option<serde_json::Value>,
    tool_calls: &[ToolCallRecord],
    citations: &[Citation],
) -> Option<serde_json::Value> {
    if tool_calls.is_empty() && citations.is_empty() {
        return images;
    }
...
async fn handle_chat_generation(
        ...
        tokio::select! {
            ...
            event = events.next() => {
                match event {
                    ...
                    Some(AgentEvent::ToolCallCompleted { id, name, success, detail, duration_ms, citations: observed }) => {
    ...
    let assistant_metadata =
        merge_metadata(image_metadata(&generated_images), &tool_calls, &citations);
