                    ...
                    Some(Ok(Message::Text(text))) => {
                        ...
                        match serde_json::from_str::<ClientMessage>(&text) {
                            Ok(ClientMessage::Send { content, metadata }) => {
                                ...
                                let task_model = chat.model_name.clone();
...
async fn handle_send_message(
                        ...
                        if let Some(choice) = stream_chunk.choices.first() {
                            ...
                            for image in &choice.delta.generated_images {
                                if generated_images.len() >= MAX_GENERATED_IMAGES
                                    || generated_images
                                        .iter()
                                        .any(|existing: &ChatImageAttachment| {
                                            existing.url == image.image_url.url
                                        })
                                {
                                    continue;
                                }

                                let Some(attachment) = generated_image_attachment(
                                    &image.image_url.url,
    ...
    if (cancelled || stream_failed) && full_content.is_empty() && generated_images.is_empty() {
        if !client_gone {
            let cancel_msg = ServerMessage::Cancelled {
                message_id: Some(assistant_message_id),
            };
    ...
    match chats::create_message_with_id(
        ...
        "assistant",
        &full_content,
        generated_image_metadata.clone(),
    )
    .await
    {
        Ok(msg) => {
