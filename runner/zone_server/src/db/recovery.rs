...
/// Bound each pass so a backlog cannot hold task locks for the whole loop.
const BATCH: i64 = 100;
...
pub async fn reconcile(pool: &PgPool) -> Result<u64, sqlx::Error> {
    ...
    let tasks: Vec<Uuid> = sqlx::query_scalar(
        "SELECT tasks.id FROM tasks JOIN task_runs ON task_runs.id=tasks.active_run_id WHERE task_runs.status IN ('completed','failed','cancelled') ORDER BY tasks.id LIMIT $1 FOR UPDATE OF tasks SKIP LOCKED",
    ).bind(BATCH).fetch_all(&mut *transaction).await?;
    let mut changed = 0;
    for task in tasks {
