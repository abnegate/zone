...
/// Authorize a writer against a run and return the run alone.
///
/// A viewer may read a run and must never answer for it, so the roles here are
/// narrower than [`read`]'s. The logs are left unread: a caller answering a
/// parked run needs `pending_question`, and nothing else the run has said.
pub async fn write(pool: &PgPool, run: Uuid, actor: Uuid) -> DbResult<Option<tasks::TaskRunRow>> {
    let mut transaction = pool.begin().await?;
    let member: Option<Uuid> = sqlx::query_scalar(
        "SELECT members.user_id FROM task_runs runs JOIN tasks ON tasks.id = runs.task_id JOIN workspace_members members ON members.workspace_id = tasks.workspace_id WHERE runs.id = $1 AND members.user_id = $2 AND members.is_active AND members.role IN ('owner', 'admin', 'member') FOR SHARE OF members",
    )
    .bind(run)
    .bind(actor)
    .fetch_optional(&mut *transaction)
    ...
    let run = tasks::get_task_run(&mut *transaction, run).await?;
    transaction.commit().await?;
    Ok(run)
}
