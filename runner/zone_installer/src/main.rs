...
const FRONTEND_DIR_ENV: &str = "ZONE_FRONTEND_DIR";
const DEFAULT_FRONTEND_DIR: &str = "frontend/build";
const DOCKER_FRONTEND_DIR: &str = "/app/frontend/build";
...
fn resolve_frontend_dir() -> PathBuf {
    if let Ok(dir) = std::env::var(FRONTEND_DIR_ENV) {
        return PathBuf::from(dir);
    }

    let docker_dir = Path::new(DOCKER_FRONTEND_DIR);
    if docker_dir.join("index.html").exists() {
        return docker_dir.to_path_buf();
    }

    PathBuf::from(DEFAULT_FRONTEND_DIR)
}
