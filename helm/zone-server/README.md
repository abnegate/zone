...
helm install zone zone/zone-server

# Install with custom values
helm install zone zone/zone-server -f values.yaml
```
...
# With custom values
helm install zone ./zone-server -f custom-values.yaml
```
...
server:
  image:
    repository: gcr.io/my-project/zone-server
    tag: "1.0.0"
  autoscaling:
    minReplicas: 3
    maxReplicas: 20
...
Install:
```bash
helm install zone ./zone-server -f production-values.yaml
```
...
Install:
```bash
helm install zone ./zone-server -f dev-values.yaml
```
...
helm upgrade zone ./zone-server -f values.yaml

# Upgrade with force migration
helm upgrade zone ./zone-server -f values.yaml --set migration.enabled=true
```
...
```bash
# Check pod status
kubectl get pods -l app.kubernetes.io/name=zone-server
